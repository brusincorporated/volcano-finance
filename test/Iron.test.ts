...
describe('Tokens', () => {
  ...
  let Lava: ContractFactory;

  before('fetch contract factories', async () => {
    Lava = await ethers.getContractFactory('Lava');
  });
  ...
  describe('Lava', () => {
    ...
    before('deploy token', async () => {
      token = await Lava.connect(creator).deploy(
        'Lava Dollar',
        'LAVA',
        creator.address,
        timelock.address
    ...
    it('adds then removes another pool', async () => {
      ...
      expect(await token.lava_pools(pool_2.address)).to.be.true;
      expect(await token.lava_pools_array(1)).to.eq(pool_2.address);

      await token.removePool(pool_2.address);
      expect(await token.lava_pools(pool_2.address)).to.be.false;
      expect(await token.lava_pools_array(1)).to.not.eq(pool_2.address);
    });
    ...
    it('sets new address for VOLCANO', async () => {
      await token.setVolcanoAddress(anyone.address);
      expect(await token.volcano_address()).to.eq(anyone.address);
    });
    ...
    it('sets new address for LAVA-to-ETH Oracle', async () => {
      await token.setLavaToEthOracle(anyone.address, ZERO_ADDR);
      expect(await token.lava_eth_oracle_address()).to.eq(anyone.address);
    });

    it('sets new address for VOLCANO-to-ETH Oracle', async () => {
      await token.setVolcanoToEthOracle(anyone.address, ZERO_ADDR);
      expect(await token.volcano_eth_oracle_address()).to.eq(anyone.address);
    });
    ...
    it('allows only owner or timelock to do restricted operations', async () => {
      ...
      await expect(token.connect(anyone_2).setRefreshCooldown(7200000)).to.be
        .reverted;
      await expect(token.connect(anyone_2).setVolcanoAddress(ZERO_ADDR)).to.be
        .reverted;
      ...
      await expect(token.connect(anyone_2).setLavaToEthOracle(ZERO_ADDR)).to.be
        .reverted;
      await expect(token.connect(anyone_2).setVolcanoToEthOracle(ZERO_ADDR)).to
        .be.reverted;
    });
  });
});
