...
async function main() {
  ...
  updateVerifyParam('Dollar', 'LAVA Stablecoin', 'LAVA', treasury.address);

  const share = await getOrCreateContract('Share', async () => {
    const Share = await ethers.getContractFactory('Share');
    const share = await Share.deploy('LAVA Share', 'VOLCANO', treasury.address);
    return await share.deployed();
  });
  updateVerifyParam('Share', 'LAVA Share', 'VOLCANO', treasury.address);
